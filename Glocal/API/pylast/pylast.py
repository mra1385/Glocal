...
def _extract_events_from_doc(doc, network):
    ...
    for node in doc.getElementsByTagName("event"):
        tmp_event = []
        tmp_event.append(Event(_extract(node, "title"), network))
        tmp_event.append(Event(_extract(node, "artist"), network))
        tmp_event.append(Event(_extract(node, "name"), network))
        tmp_event.append(Event(_extract(node, "startDate"), network))
        tmp_event.append(Event(_extract(node, "url"), network))
